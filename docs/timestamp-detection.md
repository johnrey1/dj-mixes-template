...
> **Status: implemented.** The core recommendation of these notes — swap the
> log-energy delta for a combined **MFCC + chroma checkerboard self-similarity
...
1. **One ffmpeg decode** to mono signed-16 PCM (`decode_mono_pcm`), reused for the
   waveform peaks. *(This was 8 kHz when these notes were written; the change below
   raised it to 22.05 kHz.)*
...
low-frequency resolution — **the original 8 kHz decode was too low**; chroma wants
≥ 22 kHz, which is why the decode was raised to 22.05 kHz.
...
The librosa variant (**C**) is what shipped, with three differences from the sketch
